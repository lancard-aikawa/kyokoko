...
7. **動画にする**
   ...
   **速くする手が2つある。どちらも絵は1ピクセルも変わらない。**

   ```
   python tools/build.py all --jobs auto   ショットを同時に描く（コア数の半分、最大4）
   python tools/build.py all --ff          絵づくりを ffmpeg のフィルタに任せる
   python tools/build.py all --jobs auto --ff    両方
   ```

   `--jobs` はショットが互いに関係ないことを使う。`--ff` はカメラワークが
   「静止画の上をパン・ズームするだけ」であることを使う。`--ff` が対応して
   いないショットは黙って `clip.py` に落ちるので、付けて損はない。
   ログの `[ffmpeg]` / `[PIL: 理由]` でどちらが描いたか分かる。
...
- `--jobs`  ショットは互いに関係ないので、まとめて別のプロセスに配る。
  ただし **4本あたりで頭打ち**になる。コアではなくメモリ帯域が天井。
- `--ff`  カメラワークは「静止画の上をパン・ズームするだけ」なので、
  そこは ffmpeg のフィルタの仕事にできる。文字だけ PIL に残す
  （見た目を1ピクセルも変えないため）。

`--jobs` は出力が **バイト単位で同じ**（md5 が一致する）。`--ff` は拡大の
補間器が違う（PIL は LANCZOS、ffmpeg は swscale）ぶんだけ画素が動くが、
並べても見分けはつかない。詳しくは `tools/clip_ff.py` の冒頭
（ffmpeg で踏んだ罠が6つ書いてある）。
