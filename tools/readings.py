...
TRAPS = [
    ...
    # 第005回で踏んだ組。どれも語としては正しく読めるので辞書では直せない。
    (r"(^|[、。「 ])表[にはをが]", None,
     "単独の「表」は ヒョウ と読まれる（表側・表面は正しい）", "おもて"),
    (r"(いちばん|一番)上", None,
     "「いちばん上」は イチバンジョウ になる（「丘の上」は正しい）", "いちばんうえ"),
    (r"刷ら(れ|せ)", None,
     "「刷られた」は ズラレタ になる", "印刷された または 刷《す》られた"),
    (r"空[くきけ][^間]", None,
     "「空く」は あく/すく が割れる。かな で開くか言い換える", "あく または 待つ・間が あく"),
    (r"向いて[るた]", None,
     "「向いてる」は ムカイテル になる（「向いています」「向いてます」は正しい）",
     "向《む》いてる"),
]
