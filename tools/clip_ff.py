...
GPU はほぼ効かない（PIL も x264 も CPU）。カメラワークは「静止画の上を
パン・ズームする」だけなので、そこは ffmpeg のフィルタの仕事にできる。

## どれくらい効いたか（第005回の全編・本編 10分19秒・16コア）

|  | `--jobs 1` | `--jobs 4` |
|---|---|---|
| clip.py | 24分42秒 | **11分56秒** |
| clip_ff.py | 17分00秒 | 13分53秒 |

**単独なら 1.45 倍。ただし `--jobs` とは足し算にならない。**ffmpeg は自分で
全コアを使うので、4本同時に走らせると取り合いになり、`--jobs 4` だけより
遅くなる。コアがある機械では並列化の方が効く。この版が効くのは、コアが
少ない機械と、1ショットだけ描き直すとき。
